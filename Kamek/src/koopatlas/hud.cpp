...
void dWMHud_c::loadHeaderInfo() {
	...
	SaveBlock *save = GetSaveFile()->GetBlock(-1);
	int rank = save->completions[(w + 8)][l];
	OSReport("Got Level Rank %d\n", rank);

	nw4r::lyt::Picture *MushRank_00;
	nw4r::lyt::Picture *FireRank_00;
	nw4r::lyt::Picture *StarRank_00;
	MushRank_00 = layout.findPictureByName("MushRank_00");
	FireRank_00 = layout.findPictureByName("FireRank_00");
	StarRank_00 = layout.findPictureByName("StarRank_00");
	if(rank == 0) {
		MushRank_00->SetVisible(false);
		FireRank_00->SetVisible(false);
		StarRank_00->SetVisible(false);
	}
	if(rank == 1) {
		MushRank_00->SetVisible(true);
		FireRank_00->SetVisible(false);
		StarRank_00->SetVisible(false);
	}
	if(rank == 2) {
		MushRank_00->SetVisible(false);
		FireRank_00->SetVisible(true);
		StarRank_00->SetVisible(false);
	}
	if(rank == 3) {
		MushRank_00->SetVisible(false);
		FireRank_00->SetVisible(false);
		StarRank_00->SetVisible(true);
	}
	...
	headerCol.colourise(Hue%1000, Sat, Light);
}
