...
bool CanGetAllMushrooms = false;
bool CanGetAllFlowers = false;
bool CanGetAllStars = false;
void ResetAllCompletionCandidates() {
	...
	CanGetAllMushrooms = false;
	CanGetAllFlowers = false;
	CanGetAllStars = false;
}
...
void dWMPathManager_c::setup() {
	...
	if (LastLevelPlayed[0] < 0x80) {
		...
		bool didStars = (save->completions[8][0] == 3) && (save->completions[8][1] == 3) && (save->completions[8][2] == 3) && (save->completions[8][3] == 3) && (save->completions[8][4] == 3) && (save->completions[8][5] == 3) && (save->completions[8][6] == 3) && (save->completions[8][7] == 3) && (save->completions[8][8] == 3) && (save->completions[9][0] == 3) && (save->completions[9][1] == 3) && (save->completions[9][2] == 3) && (save->completions[9][3] == 3);
		bool didFlower = false;
		bool didMushroom = false;
		if(!didStars) {
			didFlower = (save->completions[8][0] > 1) && (save->completions[8][1] > 1) && (save->completions[8][2] > 1) && (save->completions[8][3] > 1) && (save->completions[8][4] > 1) && (save->completions[8][5] > 1) && (save->completions[8][6] > 1) && (save->completions[8][7] > 1) && (save->completions[8][8] > 1) && (save->completions[9][0] > 1) && (save->completions[9][1] > 1) && (save->completions[9][2] > 1) && (save->completions[9][3] > 1);
			if(!didFlower) {
				didMushroom = (save->completions[8][0] > 0) && (save->completions[8][1] > 0) && (save->completions[8][2] > 0) && (save->completions[8][3] > 0) && (save->completions[8][4] > 0) && (save->completions[8][5] > 0) && (save->completions[8][6] > 0) && (save->completions[8][7] > 0) && (save->completions[8][8] > 0) && (save->completions[9][0] > 0) && (save->completions[9][1] > 0) && (save->completions[9][2] > 0) && (save->completions[9][3] > 0);
			}
		}
		if(didStars && CanGetAllStars) {
			completionMessageType = CMP_MSG_STARS;
		}
		else if(didFlower && CanGetAllFlowers) {
			completionMessageType = CMP_MSG_FLOWER;
		}
		else if(didMushroom && CanGetAllMushrooms) {
			completionMessageType = CMP_MSG_MUSHROOM;
		}
	}
...
dWMPathManager_c::~dWMPathManager_c() {
	...
	if (isEnteringLevel) {
		...
		// Check if we could obtain every ranks //CanGetAllMushrooms
		bool didStars = (save->completions[8][0] == 3) && (save->completions[8][1] == 3) && (save->completions[8][2] == 3) && (save->completions[8][3] == 3) && (save->completions[8][4] == 3) && (save->completions[8][5] == 3) && (save->completions[8][6] == 3) && (save->completions[8][7] == 3) && (save->completions[8][8] == 3) && (save->completions[9][0] == 3) && (save->completions[9][1] == 3) && (save->completions[9][2] == 3) && (save->completions[9][3] == 3);
		bool didFlower = false;
		bool didMushroom = false;
		if(!didStars) {
			...
			if(!didFlower) {
				didMushroom = (save->completions[8][0] > 0) && (save->completions[8][1] > 0) && (save->completions[8][2] > 0) && (save->completions[8][3] > 0) && (save->completions[8][4] > 0) && (save->completions[8][5] > 0) && (save->completions[8][6] > 0) && (save->completions[8][7] > 0) && (save->completions[8][8] > 0) && (save->completions[9][0] > 0) && (save->completions[9][1] > 0) && (save->completions[9][2] > 0) && (save->completions[9][3] > 0);
				if(!didMushroom) {
					CanGetAllMushrooms = true;
				}
				CanGetAllFlowers = true;
			}
			CanGetAllStars = true;
		}
...
void dWMPathManager_c::execute() {
	...
	if (isMoving) {
		...
		if (pressedDir >= 0) {
			...
			activatePoint();
		} else if (nowPressed & WPAD_B) { //Reset Level
			SaveBlock *save = GetSaveFile()->GetBlock(-1);
			...
			enableBypass = false;
			activatePoint();
		} else if (nowPressed & WPAD_A) { //Clear Level
			SaveBlock *save = GetSaveFile()->GetBlock(-1);
			int w = currentNode->levelNumber[0] - 1;
			int l = currentNode->levelNumber[1] - 1;
			save->completions[w][l] = 535;
			enableBypass = true;
		} else if(nowPressed & WPAD_MINUS) { //Increase Rank
			SaveBlock *save = GetSaveFile()->GetBlock(-1);
			int w = currentNode->levelNumber[0] - 1;
			int l = currentNode->levelNumber[1] - 1;
			int currentRank = save->completions[w + 8][l];
			if(currentRank < 3) {
				save->completions[w + 8][l] += 1;
				OSReport("Set rank to %d\n", currentRank + 1);
			}
			else {
				save->completions[w + 8][l] = 0;
				OSReport("Set rank to 0\n");
			}
		}
	}
}
